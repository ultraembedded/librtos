...
{
    THREAD_INIT(thread0, "thread0", thread_func, &flag0, 1);
    THREAD_INIT(thread1, "thread1", thread_func, &flag1, 1);
    THREAD_INIT(thread2, "thread2", thread_func, &flag2, 0);
    THREAD_INIT(thread3, "thread3", thread_func, &flag3, 0);

    // Deschedule for long enough for thread 0 & 1 to run
    // Thread 2 & 3 should never run as they are low priority
    // and thread 0 & 1 are busy waiting...
    thread_sleep(4);
